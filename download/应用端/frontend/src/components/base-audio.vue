...
export default {
  ...
  props: {
    ...
    name: { type: String, default: '未知音频' },
    author: { type: String, default: '未知作者' },
    autoplay: { type: Boolean, default: false },
    loop: { type: Boolean },
    obeyMuteSwitch: { type: Boolean },
    initAudio: { type: Function },
  ...
  watch: {
    ...
    src(newval) {
      this.contextInit();
      this.innerAudioContext.url = newval;
      this.audioPlay();
    }
  },
  created() {
  ...
  methods: {
    ...
    audioControls() {
      this.videoIsPlay ? this.audioPause() : this.audioPlay()
    },
    handleBtnClick() {
      this.audioControls()
    ...
    contextInit() {
      ...
      this.audioDestroy() // 防止已有音频未释放

      let innerAudioContext = uni.createInnerAudioContext()
      this.innerAudioContext = innerAudioContext
      this.destroyed = false // 重置销毁状态
