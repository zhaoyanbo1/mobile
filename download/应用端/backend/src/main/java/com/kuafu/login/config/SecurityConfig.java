...
//        http.cors().and().csrf().disable(); // ★ 开 CORS，关 CSRF（前后端分离常见做法）
        ...
        registry
                ...
                .antMatchers("/api/data/invoke/**").permitAll()
                .antMatchers("/api/user-medals").permitAll()
                .antMatchers("/api/friends/my").permitAll()
                .antMatchers("/api/team-activities").permitAll()
                .antMatchers("/api/leaderboard/page").permitAll()
                .antMatchers("/api/team-activities/**").permitAll()
