...
import com.kuafu.web.mapper.ReminderItemAllMapper;
 ...
 */
@Slf4j
@Service("ReminderItemAll")
public class ReminderItemAllServiceImpl extends ServiceImpl<ReminderItemAllMapper, ReminderItemAll> implements IReminderItemAllService {
@Autowired
private ReminderItemAllMapper taskInfoAllMapper;


@Override
public List<ReminderItemAll> selectListNew(PageRequest pageRequest, LambdaQueryWrapper<ReminderItemAll> queryWrapper) {

        LambdaQueryWrapper<ReminderItemAll> pageQueryWrapper = getPageQueryWrapper(pageRequest, queryWrapper);
  ...
  @Override
public List<ReminderItemAll> selectListNew(PageRequest pageRequest, LambdaQueryWrapper<ReminderItemAll> queryWrapper,boolean isPage) {
...
@Override
public long selectCount(PageRequest pageRequest, LambdaQueryWrapper<ReminderItemAll> queryWrapper) {
        LambdaQueryWrapper<ReminderItemAll> pageQueryWrapper = getPageQueryWrapper(pageRequest, queryWrapper);
...
@Override
public IPage pageNew(IPage<ReminderItemAll> page, PageRequest pageRequest, LambdaQueryWrapper<ReminderItemAll> queryWrapper) {
